...
def generate_kitti_odometry_index(
    data_root: str | Path,
    ...
    records = []
    for sequence in tqdm(sequence_names, desc="[KITTIOdometry] building index", unit="sequence"):
        sequence_dir = sequences_root / sequence
        calib = _parse_kitti_calib(sequence_dir / "calib.txt")
        poses = _parse_kitti_poses(poses_root / f"{sequence}.txt")
        frames = []
