...
所有剩余同名点。如果任一 view 没有真实 depth，或者 dataloader 将 depth 标记为
`placeholder_missing_dense_depth`，`mixed` 会跳过 `geom`，只走 `feat`。
这种 no-depth 模式下，默认 SIFT feat 路径会使用 descriptor matching，
匹配过滤阈值由 `--match-ratio 0.75` 控制；只有存在真实 depth 时才使用 GT projection。
...
提供 `pixel_rays` 和 `ray_distance` 的 view 会走通用 ray-camera 投影路径，
不再走 pinhole `K`。`--ray-angular-thresh` 控制这类同名点的最近 ray 过滤阈值。
